# XMC&trade; MCU: FreeRTOS blinky

[View this README on GitHub.](https://github.com/Infineon/mtb-example-xmc-blinky-freertos)

[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example will clone the "[FreeRTOS Blinky](https://github.com/Infineon/mtb-example-xmc-freertos-blinky)" application with the desired name "FreeRTOSBlinky" configured for the *KIT_XMC47_RELAX_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC47_RELAX_V1 --app-id mtb-example-xmc-freertos-blinky --user-app-name FreeRTOSBlinky --target-dir "C:/mtb_projects"
   ```
...
This example toggles an LED using FreeRTOS tasks. The “**pin 1.0**” personality is used here to configure the LED pin in **Device Configurator** as shown in Figure 1. The toggle period is defined by USER_LED_TOGGLE_PERIOD_MS.

**Figure 1. LED pin configuration**
![1](images/pin.png)

The main function consists of two tasks (blinky_task and main_task), a semaphore, and a FreeRTOS scheduler.
The FreeRTOS scheduler decides which tasks should be executed and when. It is started by **vTaskStartScheduler(). 
...
Document title: *CE231920* - *XMC&trade; MCU: FreeRTOS blinky*
